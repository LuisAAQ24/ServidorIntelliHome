...
class ChatServer:
    def __init__(self, host='0.0.0.0', port=6060):
        try:
            ...
            self.http_server = HTTPServer(('localhost', 6061), ConfirmationHandler)
                ...
                if message:
                    ...
                    if len(message_parts) < 1:
                        ...
                        if tipo_mensaje == "registro":
                            ...
                            if self.send_confirmation_email(dato1_cliente):
                                response = "true\n"
                            else:
                                response = "false\n"
                        else:
    ...
    def handle_registration(self, message_parts):
        email = message_parts[1]  # Email
        contrasena = message_parts[2]  # Contraseña

        # Verificar si ya existe el email o contraseña
...
class ConfirmationHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if username:
            ...
            # Notificar a todos los clientes que el usuario ha confirmado su registro
            for client in server.clients:
                try:
                    client.send(f"El usuario ha confirmado su registro\n".encode('utf-8'))
                except Exception as e:
